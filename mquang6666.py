...
def run(sdt):
    ...
    while True:
        for api in apis:
            try:
                api(sdt)
                print(f"{luc}[+] Đã gửi thành công {api.__name__} đến số {sdt}.{hong}")
            except Exception as e:
                print(f"{do}[!] Lỗi Khi gửi {api.__name__}: {str(e)}.{hong}")
...
def chay():
    ...
    if sdt.isdigit() and len(sdt) == 10:
        ...
        print(f"{do}[!] Số điện thoại không hợp lệ. Vui lòng thử lại.{luc}")
